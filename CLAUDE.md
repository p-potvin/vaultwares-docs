...
**COMET IS STOPPED (Thu, 06 Aug 2026 02:10) — bandwidth quota.** We have used **6000 GB of the allotted 8192 GB**, and the quota does not reset until **12 Nov 2026**. The `vw-comet` stack on `vps-ovhcloud` (`comet`, `comet-postgres`, `comet-vpn`) was stopped with `docker compose stop`, so `100.67.25.118:5173` refuses connections and will stay down across reboots. **Do not restart it, and do not work around it by calling debrid or indexer endpoints directly** — that is exactly what gets the account IP-banned, and the remaining 2192 GB has to last until November. If a task needs Comet, stop and ask the user first. To bring it back once authorised: `ssh ubuntu@100.67.25.118 'cd /opt/vw-comet && sudo docker compose start'`.
