...
| NETWORK_INFRASTRUCTURE | Any networking, SSH, VPN, VPS, ports, CI runner egress | instructions/summaries/NETWORK_INFRASTRUCTURE.md | instructions/notes/NETWORK_INFRASTRUCTURE.md | tailnet,tailscale,ssh,vps,nginx,ports,runner,brume,tinyproxy,proxy,egress,cloudflare,stream,residential,playback,403,cf-mitigated,api/stream |
| NETWORK_ROUTING_POSTMORTEM | History on routing, NGINX CORS, DNS, SNI misrouting, IP binds | docs-content/operations/network-routing-cors-postmortem.mdx | — | dns,routing,cors,deployments,tailscale |
