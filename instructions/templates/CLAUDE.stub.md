...
Estimate step (mandatory): compute `estimated_output_tokens` after reading required summaries; if >=8000 apply overlay `LONG_RUNNING_TASKS` and persist `VW_STATE` in the ledger (do not paste VW_STATE into chat unless the user explicitly asks). For resume, include a `VW_STATE_REF` pointer (resumeId + ledger event path/hash).
