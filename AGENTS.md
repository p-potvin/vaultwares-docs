...
- **Torrent & Debrid Policy:** Three separate torrenting/streaming entities. Each has its own Real-Debrid token and its own **Prowlarr tag** — the tag selects the provider set, so it must be correct per entity.
  ...
  2. **Media stack** — its own Real-Debrid token. Reaches Prowlarr, decypharr, qBittorrent and SABnzbd directly. Prowlarr tag: `flaresolverr`.
...
- **No git worktrees — branch instead.** Worktrees scatter checkouts outside the repo (`Documents\Codex`, `AppData\Local\Temp`, `.config\superpowers`), so they are invisible both in the repo and when browsing GitHub, and they accumulate. They also do NOT receive gitignored files: a worktree build of vault-monitor on 2026-09-10 had no `.env`, silently baked the absolute `https://api.vaultwares.ca` base into the bundle, and broke every tab of the monitor with CORS errors for 11 days. Use `git switch -c <branch>` in the repo itself. If a second working copy is genuinely unavoidable, clone to a sibling directory and delete it when done.
